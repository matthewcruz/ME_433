...
public class SerialConsoleActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    volatile int RedThresh = 35*255/100;
    volatile int GreenThresh = 17*255/100;
...
//                    }
            ...
            if (wbTotal>0) {
                COM_OUT = COM * 100 / 640;
                String sendString = String.valueOf(COM_OUT) + "\n";
                try {
                    sPort.write(sendString.getBytes(), 10); // 10 is the timeout
                } catch (IOException e1) {
                }
...
//                showStatus(mDumpTextView, "RTS - Request To Send", sPort.getRTS());
                ...
                String sendString = String.valueOf(RedThresh) + "\n";
                try {
                    sPort.write(sendString.getBytes(),10); // 10 is the timeout
                }
                catch (IOException e1) {}
...
//        mScrollView.smoothScrollTo(0, mDumpTextView.getBottom());
        byte[] sData = {'a',0};
        try { sPort.write(sData, 10); } catch (IOException e) { }
    }
