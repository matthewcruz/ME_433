    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_MOUSE_EMULATE:
            ...
            if(appData.emulateMouse)
            {
                ...
                //added
                I2C_multiread(0x6B, 0x20, data, 14); 
                ...
                if(movement_length > 50)
                {
                    ...
                    appData.xCoordinate =(int8_t)(9.81*4*((float)(accel_x + (0xFFFF/2)+1)/0xFFFF) - 2*9.81);  //(int8_t)//dir_table[vector & 0x07] ;
                    appData.yCoordinate =(int8_t)(9.81*4*((float)(accel_y + (0xFFFF/2)+1)/0xFFFF)- 2*9.81);  //(int8_t)//dir_table[(vector+2) & 0x07];
                    vector ++;
                    movement_length = 0;    
                }
            }
